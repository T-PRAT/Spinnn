...
<template>
  <div class="space-y-4">
    ...
    <div class="h-[400px] max-h-[600px] overflow-y-auto space-y-3">
      <div v-for="category in workoutCategories" :key="category.id" class="space-y-1.5">
        ...
        >
          <div class="flex items-center gap-2">
            ...
            <span class="font-semibold text-sm text-foreground">{{ category.name }}</span>
          </div>
          <svg
            :class="['w-4 h-4 text-muted-foreground transition-transform', expandedCategories.has(category.id) ? 'rotate-180' : '']"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <div v-if="expandedCategories.has(category.id)" class="space-y-1.5 pl-1">
          <div
            ...
            :class="[
              ...
              selectedWorkout?.id === workout.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50 hover:bg-accent/50',
            ]"
          >
