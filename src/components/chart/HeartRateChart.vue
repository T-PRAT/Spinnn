...
function initChart() {
  ...
  yScale = d3.scaleLinear().range([height, 0]).domain([40, 200]); // Typical HR range
  ...
  // Add X axis
  xAxisGroup = svg.append("g").attr("transform", `translate(0,${height})`).attr("class", "x-axis");
  ...
  // Add the line path
  path = svg.append("path").attr("fill", "none").attr("stroke", "#ef4444").attr("stroke-width", 2);

  // Add grid lines
  svg.append("g").attr("class", "grid").attr("opacity", 0.1).call(d3.axisLeft(yScale).tickSize(-width).tickFormat(""));
}
...
function updateChart(data) {
  ...
  path.datum(visibleData).transition().duration(100).attr("d", line);
...
<template>
  <div class="bg-white rounded-lg p-6 border border-gray-200">
    ...
    <div v-if="dataPoints.length === 0" class="text-center text-gray-500 py-12">No heart rate data yet</div>
  </div>
</template>
