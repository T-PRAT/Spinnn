...
onMounted(() => {
  // Try to load saved workout state
...
<template>
  <div class="max-w-screen-2xl mx-auto px-2 space-y-4">
    ...
    <div v-if="!appState.mockModeActive.value" class="flex items-center justify-center gap-6">
      <!-- HRM status -->
      <button
      ...
      >
        ...
        >
          {{ deviceStatus.hrm.connecting ? 'Cardino...' : deviceStatus.hrm.connected ? 'Cardino' : 'Cardino deconnecte' }}
        </span>
      </button>
    ...
    <!-- Large chart -->
    <div class="bg-card rounded-lg p-2 shadow border border-border min-h-[500px]">
      <WorkoutChart
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <!-- Cadence (petite) -->
      <div class="bg-card rounded-lg p-3 shadow border border-border text-center">
        <div class="text-xs text-muted-foreground mb-1">Cadence</div>
        <div class="text-xl md:text-2xl font-semibold text-chart-2">
          {{ currentMetrics.cadence }}<span class="text-xs md:text-sm ml-1">rpm</span>
        </div>
      </div>

      <!-- PUISSANCE (GRANDE) -->
      <div class="bg-card rounded-lg p-6 shadow border border-border text-center">
        <div class="text-sm text-muted-foreground mb-2">Puissance</div>
        <div class="text-4xl md:text-6xl font-bold text-chart-1">
          {{ currentMetrics.power }}<span class="text-xl md:text-2xl ml-1">W</span>
        </div>
      </div>

      <!-- FC (GRANDE) -->
      <div class="bg-card rounded-lg p-6 shadow border border-border text-center">
        <div class="text-sm text-muted-foreground mb-2">FC</div>
        <div class="text-4xl md:text-6xl font-bold text-destructive">
          {{ currentMetrics.heartRate }}<span class="text-xl md:text-2xl ml-1">bpm</span>
        </div>
      </div>

      <!-- Vitesse (petite) -->
      <div class="bg-card rounded-lg p-3 shadow border border-border text-center">
        <div class="text-xs text-muted-foreground mb-1">Vitesse</div>
        <div class="text-xl md:text-2xl font-semibold text-primary">
          {{ (currentMetrics.speed * 3.6).toFixed(1) }}<span class="text-xs md:text-sm ml-1">km/h</span>
        </div>
      </div>
    </div>
    ...
    >
      <div class="bg-card border border-border rounded-lg p-6 max-w-md mx-4 shadow-lg">
        ...
        <p class="text-muted-foreground mb-6">
          Êtes-vous sûr de vouloir arrêter la séance ? Vos données seront sauvegardées.
        </p>
